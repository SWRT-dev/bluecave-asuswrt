...
update_mac_address()
{
	...
	[ "$radio_name" = "wlan1" ] && phy_offset=4
	[ "$radio_name" = "wlan2" ] && phy_offset=8
	...
	# Increment the last byte by the the proper incrementation according to the physical interface (wlan0/wlan1/wlan2)
...
update_enable_online()
{
	...
	[ "$enable_online_changed" -gt 0 ] && echo "restart_${radio_name}=yes" >> ${CONF_DIR}/${RESTART_FLAG}_${radio_name}
}
