...
update_acl_list()
{
	...
	if [ ! -e /tmp/maclist_x.txt ]
	then
		echo "" > /tmp/maclist_x.txt
	fi
	db_acl_list=`cat /tmp/maclist_x.txt`
	db_acl_list="${db_acl_list//,/ }"
	
	echo "update_acl_list: " $db_acl_list > /tmp/fapi_debug.txt
	if [ -n "$db_acl_list" ]
	then
