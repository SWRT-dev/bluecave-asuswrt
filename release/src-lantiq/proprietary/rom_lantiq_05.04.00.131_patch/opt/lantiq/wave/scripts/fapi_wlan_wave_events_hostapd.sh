...
#	fi
	...
	# CancelWPS on all other interfaces. - No need here?
	#handle_extra_actions $interface_name
}
...
#	# Wait for the station to connect, then call handle_extra_actions
	echo "wps_success, call handle_extra_actions" > /dev/console
	#Arad: TODO: CancelWPS on all other interfaces.
	handle_extra_actions $interface_name
#	ech0 "$mac_addr" > /dev/console
#	ech0 "$mac_addr" > /tmp/wps_success_${interface_name}
}
...
#		mac_addr_wps=`cat /tmp/wps_success_${interface_name}`
#		echo "mac_addr_wps=$mac_addr_wps" > /dev/console
#		if [ -n "$mac_addr_wps" ] && [ "$mac_addr_wps" = "$mac_address" ]
#		then
#			echo "Same macs, call handle_extra_actions" > /dev/console
#			handle_extra_actions $interface_name
#		fi
#
