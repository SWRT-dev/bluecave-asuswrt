/*
 * This path manager is a variant of the "fullmesh" path manager that
 * at first only opens a subflow on the network interface marked as
 * the 'master' interface.  Subflows will be opened on non-master
 * interfaces upon receiving an address advertisement (ADD_ADDR) from
 * an MPTCP server.
 *
 * TODO: Most of the code is common to the original "fullmesh"
 *       implementation, and differs mostly in the subflow worker.
 *       Refactor all code common to both "fullmesh" and
 *       "fullmesh_mf".
 */
...
/* Maximum number subflows allowed per connection. */
#define MPTCP_MAX_SUBFLOWS 2
...
next_subflow:
	...
	if (master_tp && master_tp->mptcp && master_tp->mptcp->master && fmp->first_pair == 0) {
			...
			rem.addr = inet6_sk(meta_sk)->daddr;
			rem.port = inet_sk(meta_sk)->inet_dport;
	...
	if (event->code == MPTCP_EVENT_DEL) {
		...
		if (j < 0) {
			...
			if (event->family == AF_INET &&
			    ...
			    event->master == mptcp_local->locaddr4[i].master)
				goto duno;

			if (event->family == AF_INET6 &&
			    event->low_prio == mptcp_local->locaddr6[i].low_prio &&
			    event->master == mptcp_local->locaddr6[i].master)
				goto duno;
		}
		...
		if (event->family == AF_INET) {
			...
			mptcp_local->locaddr4[i].master = event->master;
		} else {
			...
			mptcp_local->locaddr6[i].master = event->master;
		}
...
duno:
	...
	for (i = 0; i < MPTCP_HASH_SIZE; i++) {
		...
		hlist_nulls_for_each_entry_rcu(meta_tp, node, &tk_hashtable[i],
			...
			else if (event->family == AF_INET && ipv6_only_sock(meta_sk))
				continue;
				...
				struct net *net)
{
	...
	mpevent.master = (netdev->flags & IFF_MPMASTER) ? 1 : 0;
	...
	mptcp_debug("%s created event for %pI4, code %u prio %u master %u\n",
		    __func__, &ifa->ifa_local, mpevent.code,
		   mpevent.low_prio, mpevent.master);
				...
				struct net *net)
{
	...
	mptcp_debug("%s created event for %pI6, code %u prio %u master %u\n",
		    __func__, &ifa->addr, mpevent.code,
		    mpevent.low_prio, mpevent.master);
	add_pm_event(net, &mpevent);
...
static void full_mesh_new_session(const struct sock *meta_sk)
{
	...
	if (family == AF_INET) {
		...
		master_tp->mptcp->master = mptcp_local->locaddr4[index].master;
	} else {
		master_tp->mptcp->low_prio = mptcp_local->locaddr6[index].low_prio;
		master_tp->mptcp->master = mptcp_local->locaddr6[index].master;
	}
	...
	if (!meta_v4 && ipv6_only_sock(meta_sk))
		goto skip_ipv4;
...
static void full_mesh_release_sock(struct sock *meta_sk)
{
	...
	mptcp_for_each_bit_set(mptcp_local->loc4_bits, i) {
		...
		mptcp_for_each_sk(mpcb, sk) {
			...
			mptcp_local->locaddr4[i].master = tp->mptcp->master;
		}
...
static int mptcp_fm_seq_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "Index, Address-ID, Backup, Master, IP-address\n");
	...
	mptcp_for_each_bit_set(mptcp_local->loc4_bits, i) {
		...
		seq_printf(seq, "%u, %u, %u, %u, %pI4\n", i, loc4->loc4_id,
			   loc4->low_prio, loc4->master, &loc4->addr);
	}
	...
	mptcp_for_each_bit_set(mptcp_local->loc6_bits, i) {
		...
		seq_printf(seq, "%u, %u, %u, %u, %pI6\n", i, loc6->loc6_id,
			   loc6->low_prio, loc6->master, &loc6->addr);
	}
	rcu_read_unlock_bh();
...
static int mptcp_fm_init_net(struct net *net)
{
	...
	if (!proc_create("mptcp_fullmesh_mf", S_IRUGO, net->proc_net,
			 &mptcp_fm_seq_fops)) {
		err = -ENOMEM;
		goto err_seq_fops;
	}
...
static int __init full_mesh_register(void)
{
	...
	if (num_subflows > MPTCP_MAX_SUBFLOWS) {
		mptcp_debug("%s reduced number of requested subflows (%d) to the maximum allowed (%d)\n",
			__func__,
			num_subflows,
			MPTCP_MAX_SUBFLOWS);

		num_subflows = MPTCP_MAX_SUBFLOWS;
	}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Full Mesh (master first) MPTCP");
MODULE_VERSION("0.88");
